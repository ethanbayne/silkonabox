...
sudo apt-get -qq -y install glib2.0 libglib2.0-dev libpcap-dev g++ libpython2.7-dev make gcc unzip
...
if which rwp2yaf2silk > /dev/null; then
	...
	if [ ! -f libfixbuf-$lfbversion.tar.gz ]; then
    		...
    		wget https://tools.netsa.cert.org/releases/libfixbuf-$lfbversion.tar.gz
      else
	...
	if [ ! -f yaf-$yafversion.tar.gz ]; then
    		...
    		wget https://tools.netsa.cert.org/releases/yaf-$yafversion.tar.gz
      else
	...
	if [ ! -f silk-$silkversion.tar.gz ]; then
    		...
    		wget https://tools.netsa.cert.org/releases/silk-$silkversion.tar.gz
      else
  ...
  echo -e "$(tput setaf 6)Building YAF...$(tput sgr0)"
	...
	./configure --enable-applabel --with-libpcap
	make
	sudo make install
